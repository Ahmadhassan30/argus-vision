"""Agent B: ViT-B/16 classifier for ISIC-8 dermoscopic image classification.

This module defines :class:`AgentB`, the second participant in the Argus Vision
adversarial debate. It wraps a ``timm`` Vision Transformer (ViT-B/16) backbone
with an 8-class head and exposes a clean inference API returning a
...
# The timm model identifier for the ViT-B/16 backbone.
MODEL_NAME: str = "vit_base_patch16_224.augreg_in21k_ft_in1k"


class AgentB:
    """ViT-B/16 based classifier (Agent B) for the Argus Vision debate.

    Agent B loads a Vision Transformer (ViT-B/16) model from ``timm`` with an
    8-class head. A trained checkpoint is loaded when available; otherwise, if
    ...
    Attributes:
        agent_id: Stable identifier for this agent, always ``"B"``.
    ...
    def __init__(
        ...
        """Initialize Agent B and load its weights.
        ...
        if checkpoint_path is not None and os.path.exists(checkpoint_path):
            logger.info(
                "Agent B: loading checkpoint from '%s' onto device '%s'.",
                checkpoint_path,
                self.device,
            )
            ...
            if load_result.missing_keys:
                logger.warning(
                    "Agent B: %d missing key(s) when loading checkpoint: %s",
                    len(load_result.missing_keys),
                    load_result.missing_keys,
                )
            if load_result.unexpected_keys:
                logger.warning(
                    "Agent B: %d unexpected key(s) when loading checkpoint: %s",
            ...
            raise ModelNotLoadedError(
                f"Agent B: no checkpoint found at '{checkpoint_path}' and "
    ...
    def get_model(self) -> nn.Module:
        ...
        This is required by attention rollout, which needs direct access to the
        transformer blocks to register hooks on their attention modules.

        Returns:
            The wrapped ViT-B/16 :class:`torch.nn.Module`.
        """
        return self.model
